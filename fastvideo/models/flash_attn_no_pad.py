...
def npu_flash_attn_varlen_qkvpacked_func(
    qkv,
    cu_seqlens,
    max_seqlen,
    dropout_p=0.0,
    ...
    flash_attn_varlen_qkvpacked_func融合算子替换
    ...
    if causal:
        atten_mask_npu = torch.triu(torch.ones([2048, 2048]), diagonal=1).bool().to(torch.device('npu'))
        head_num = q.shape[1]
...
def flash_attn_no_pad(qkv, key_padding_mask, causal=False, dropout_p=0.0, softmax_scale=None):
    ...
    output_unpad = npu_flash_attn_varlen_qkvpacked_func(
        x_unpad,
        cu_seqlens,
        max_s,
        dropout_p,
